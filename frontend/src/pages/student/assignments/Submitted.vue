<script setup lang="ts">
import AppSidebar from "@/components/sidebars/AppSidebar.vue"
import {
  SidebarProvider,
  SidebarInset,
  SidebarTrigger,
} from "@/components/ui/sidebar"

import {
  Breadcrumb,
  BreadcrumbList,
  BreadcrumbItem,
  BreadcrumbLink,
  BreadcrumbSeparator,
  BreadcrumbPage,
} from "@/components/ui/breadcrumb"

import { Separator } from "@/components/ui/separator"

import {
  Card,
  CardHeader,
  CardTitle,
  CardDescription,
  CardContent,
} from "@/components/ui/card"

import { Button } from "@/components/ui/button"
import { Badge } from "@/components/ui/badge"

import {
  Bell,
  CheckCircle,
  FileText,
  Calendar,
  Eye,
  Filter,
  Search,
  Download,
  Clock,
} from "lucide-vue-next"

const userRole = "Student"

// Mock Submitted Assignments Data
const submittedAssignments = [
  {
    id: 1,
    title: "Chemical Reactions Lab Report",
    subject: "Science",
    teacher: "Ms. Ana Reyes",
    submittedDate: "Feb 10, 2025",
    dueDate: "Feb 12, 2025",
    status: "graded",
    grade: "95/100",
    percentage: 95,
    type: "Lab Report",
    feedback: "Excellent analysis and clear presentation of results.",
    submittedOn: "On Time",
  },
  {
    id: 2,
    title: "Algebraic Expressions Worksheet",
    subject: "Mathematics",
    teacher: "Mr. Juan dela Cruz",
    submittedDate: "Feb 8, 2025",
    dueDate: "Feb 9, 2025",
    status: "graded",
    grade: "88/100",
    percentage: 88,
    type: "Worksheet",
    feedback: "Good work. Review problems 15-18 for improvement.",
    submittedOn: "On Time",
  },
  {
    id: 3,
    title: "Shakespeare Analysis Essay",
    subject: "English",
    teacher: "Mrs. Sofia Garcia",
    submittedDate: "Feb 12, 2025",
    dueDate: "Feb 10, 2025",
    status: "graded",
    grade: "82/100",
    percentage: 82,
    type: "Essay",
    feedback: "Good insights but needs better structure. Late submission.",
    submittedOn: "Late",
  },
  {
    id: 4,
    title: "Philippine Revolution Timeline",
    subject: "Social Studies",
    teacher: "Ms. Maria Lopez",
    submittedDate: "Feb 14, 2025",
    dueDate: "Feb 14, 2025",
    status: "pending",
    grade: null,
    percentage: null,
    type: "Project",
    feedback: null,
    submittedOn: "On Time",
  },
  {
    id: 5,
    title: "Poetry Analysis - Florante at Laura",
    subject: "Filipino",
    teacher: "Mr. Pedro Santos",
    submittedDate: "Feb 11, 2025",
    dueDate: "Feb 12, 2025",
    status: "pending",
    grade: null,
    percentage: null,
    type: "Essay",
    feedback: null,
    submittedOn: "On Time",
  },
  {
    id: 6,
    title: "Geometry Problem Set",
    subject: "Mathematics",
    teacher: "Mr. Juan dela Cruz",
    submittedDate: "Feb 5, 2025",
    dueDate: "Feb 6, 2025",
    status: "graded",
    grade: "100/100",
    percentage: 100,
    type: "Problem Set",
    feedback: "Perfect score! Excellent work on all problems.",
    submittedOn: "On Time",
  },
]

const getStatusBadge = (status: string) => {
  if (status === "graded") {
    return { variant: "default" as const, label: "Graded", class: "bg-green-500" }
  } else {
    return { variant: "secondary" as const, label: "Pending Review", class: "bg-yellow-500" }
  }
}

const getGradeColor = (percentage: number | null) => {
  if (percentage === null) return "text-gray-500"
  if (percentage >= 90) return "text-green-600"
  if (percentage >= 80) return "text-blue-600"
  if (percentage >= 75) return "text-yellow-600"
  return "text-red-600"
}

const getSubjectColor = (subject: string) => {
  const colors: Record<string, string> = {
    Mathematics: "text-blue-600 bg-blue-50",
    Science: "text-green-600 bg-green-50",
    English: "text-purple-600 bg-purple-50",
    Filipino: "text-red-600 bg-red-50",
    "Social Studies": "text-orange-600 bg-orange-50",
  }
  return colors[subject] || "text-gray-600 bg-gray-50"
}
</script>

<template>
  <SidebarProvider>
    <AppSidebar :userRole="userRole" />

    <SidebarInset>
      <!-- HEADER -->
      <header
        class="flex h-16 shrink-0 items-center gap-2 px-4 
               transition-[width,height] ease-linear"
      >
        <SidebarTrigger class="-ml-1" />
        <Separator orientation="vertical" class="mx-2 h-4" />

        <Breadcrumb>
          <BreadcrumbList>
            <BreadcrumbItem class="hidden md:block">
              <BreadcrumbLink href="/student/dashboard">
                Student
              </BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator class="hidden md:block" />
            <BreadcrumbItem class="hidden md:block">
              <BreadcrumbLink href="/student/assignments/pending">
                Assignments
              </BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator class="hidden md:block" />
            <BreadcrumbItem>
              <BreadcrumbPage>Submitted</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>

        <div class="ml-auto flex items-center gap-2 px-4">
          <Button variant="outline" size="sm">
            <Bell class="h-4 w-4" />
          </Button>
        </div>
      </header>

      <!-- MAIN CONTENT -->
      <div class="flex flex-1 flex-col gap-4 p-4 pt-0">
        <!-- Page Header -->
        <div class="rounded-lg border bg-card p-6">
          <div
            class="flex flex-col md:flex-row md:items-center md:justify-between gap-4"
          >
            <div>
              <h2 class="text-3xl font-bold">Submitted Assignments</h2>
              <p class="text-muted-foreground mt-2">
                You have submitted {{ submittedAssignments.length }} assignments
              </p>
            </div>

            <div class="flex gap-2">
              <Button variant="outline">
                <Filter class="mr-2 h-4 w-4" />
                Filter
              </Button>
              <Button variant="outline">
                <Calendar class="mr-2 h-4 w-4" />
                Sort by Date
              </Button>
            </div>
          </div>
        </div>

        <!-- Search Bar -->
        <Card>
          <CardContent class="p-4">
            <div class="relative">
              <Search
                class="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-muted-foreground"
              />
              <input
                type="text"
                placeholder="Search submitted assignments..."
                class="w-full pl-10 pr-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
              />
            </div>
          </CardContent>
        </Card>

        <!-- Assignments List -->
        <div class="space-y-4">
          <Card
            v-for="assignment in submittedAssignments"
            :key="assignment.id"
            class="hover:shadow-lg transition-shadow"
          >
            <CardHeader>
              <div class="flex items-start justify-between gap-4">
                <div class="flex-1">
                  <div class="flex items-center gap-2 mb-2">
                    <Badge :class="getSubjectColor(assignment.subject)">
                      {{ assignment.subject }}
                    </Badge>
                    <Badge :variant="getStatusBadge(assignment.status).variant">
                      {{ getStatusBadge(assignment.status).label }}
                    </Badge>
                    <Badge variant="outline">{{ assignment.type }}</Badge>
                    <Badge
                      v-if="assignment.submittedOn === 'Late'"
                      variant="destructive"
                    >
                      Late Submission
                    </Badge>
                  </div>
                  <CardTitle class="text-xl">{{ assignment.title }}</CardTitle>
                  <CardDescription class="mt-2">
                    Teacher: {{ assignment.teacher }}
                  </CardDescription>
                </div>

                <div class="text-right">
                  <div
                    v-if="assignment.grade"
                    :class="[
                      'text-2xl font-bold',
                      getGradeColor(assignment.percentage),
                    ]"
                  >
                    {{ assignment.grade }}
                  </div>
                  <div v-else class="text-2xl font-bold text-gray-400">
                    ---
                  </div>
                  <div class="text-xs text-muted-foreground">
                    {{ assignment.status === "graded" ? "Grade" : "Pending" }}
                  </div>
                </div>
              </div>
            </CardHeader>

            <CardContent>
              <div class="space-y-4">
                <div class="flex flex-wrap gap-4 text-sm text-muted-foreground">
                  <div class="flex items-center gap-2">
                    <CheckCircle class="h-4 w-4" />
                    <span>Submitted: {{ assignment.submittedDate }}</span>
                  </div>
                  <div class="flex items-center gap-2">
                    <Clock class="h-4 w-4" />
                    <span>Due: {{ assignment.dueDate }}</span>
                  </div>
                </div>

                <div
                  v-if="assignment.feedback"
                  class="p-3 bg-muted/50 rounded-lg border"
                >
                  <p class="text-sm font-semibold mb-1">Teacher's Feedback:</p>
                  <p class="text-sm text-muted-foreground">
                    {{ assignment.feedback }}
                  </p>
                </div>

                <div
                  v-else
                  class="p-3 bg-yellow-50 border border-yellow-200 rounded-lg"
                >
                  <p class="text-sm text-yellow-800">
                    <strong>Note:</strong> Your teacher is currently reviewing
                    your submission. Feedback will be available soon.
                  </p>
                </div>

                <div class="flex gap-2">
                  <Button variant="outline" size="sm">
                    <Eye class="mr-2 h-4 w-4" />
                    View Submission
                  </Button>
                  <Button variant="outline" size="sm">
                    <Download class="mr-2 h-4 w-4" />
                    Download
                  </Button>
                  <Button
                    v-if="assignment.status === 'graded'"
                    variant="outline"
                    size="sm"
                  >
                    <FileText class="mr-2 h-4 w-4" />
                    View Rubric
                  </Button>
                </div>
              </div>
            </CardContent>
          </Card>
        </div>

        <!-- Empty State (Hidden when there are assignments) -->
        <Card v-if="submittedAssignments.length === 0" class="p-12">
          <div class="text-center">
            <FileText class="h-16 w-16 text-muted-foreground mx-auto mb-4" />
            <h3 class="text-xl font-semibold mb-2">No Submitted Assignments</h3>
            <p class="text-muted-foreground">
              You haven't submitted any assignments yet.
            </p>
          </div>
        </Card>
      </div>
    </SidebarInset>
  </SidebarProvider>
</template>